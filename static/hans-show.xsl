<xsl:transform
  ...
  <xsl:template match="vka:A" mode="narrative">
    ...
    <xsl:result-document href="#content" method="ixsl:append-content">
      ...
      <div class="vka_A">
        <xsl:if test="not(empty(vka:tgrp/vka:arakiri))"><div class="vka_arakiri"><xsl:for-each select="tokenize(vka:tgrp/vka:arakiri, '(&lt;/p&gt;|&lt;p&gt;)+')"><p><xsl:value-of select="."/></p></xsl:for-each></div></xsl:if>
        ...
        <xsl:if test="not(empty(vka:tgrp/vka:kirjeldus))">
          <div class="vka_kirjeldus"><h3>Tekstile lisatud kirjeldus:</h3><xsl:for-each select="tokenize(vka:tgrp/vka:kirjeldus, '(&lt;/p&gt;|&lt;p&gt;)+')"><p><xsl:value-of select="."/></p></xsl:for-each></div>
        </xsl:if>
        <xsl:if test="not(empty(vka:tgrp/vka:tkom))"><div class="vka_tkom"><h3>Teksti ja selle konteksti seletavaid kommentaare:</h3><xsl:for-each select="tokenize(vka:tgrp/vka:tkom, '(&lt;/p&gt;|&lt;p&gt;)+')"><p><xsl:value-of select="."/></p></xsl:for-each></div></xsl:if>
