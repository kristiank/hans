  ...
  <xsl:template match="vka:A" mode="narrative">
    ...
    <xsl:result-document href="#content" method="ixsl:append-content">
      ...
      <div class="vka_A">
        ...
        <xsl:if test="not(empty(vka:tgrp/vka:kirjeldus))"><div class="vka_kirjeldus">Tekstile on lisatud kirjeldus:<br/> <xsl:value-of select="vka:tgrp/vka:kirjeldus"/></div></xsl:if>
        <xsl:if test="not(empty(vka:tgrp/vka:tkom))"><div class="vka_tkom">Teksti ja selle konteksti seletavaid kommentaare:<br/> <xsl:value-of select="vka:tgrp/vka:tkom"/></div></xsl:if>
